...
**Проще всего:** открой папку `ai-lab/` в Codex и разреши (Trust) проект —
сервер подтянется сам из готового `ai-lab/.codex/config.toml`, а правила из
`ai-lab/AGENTS.md`. Больше ничего править не надо.

**Вручную** (если не открываешь эту папку) — добавь блок в `config.toml`:
