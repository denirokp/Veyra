# ai-lab-mcp → MCP Hub — план онбординга

Рабочий чек-лист для регистрации `ai-lab-mcp` в MCP Hub Avito. Закрывает
гэпы **G2** (ai-lab-mcp не Hub-ready) и **G3** (нет PII-анонимизации) разом:
...
## Готовность `ai-lab-mcp` — что уже ок
...
В коде `ai-lab-mcp/server.py`, скорее всего, **менять ничего не нужно**:
OAuth берёт на себя Hub, своей авторизации не делаем; внутренний токен
`ai-lab-mcp → backend` (`AILAB_BACKEND_TOKEN`) остаётся как есть.
...
- [ ] **Задеплоить `ai-lab-mcp` на Avito PaaS** — нужен стабильный URL,
      на него ссылается запись в registry. Сейчас сервер standalone (`:8765`).
- [ ] Задеплоить backend-движок (`app.main`) — `ai-lab-mcp` проксирует к нему.
- [ ] Подготовить запись в `mcp-registry` (черновик ниже).
- [ ] Запросить **PII-анонимизацию** для `ai-lab` — корпус документов может
      содержать ПДн; Hub прогонит ответы через NER-модель.
- [ ] Открыть PR в `mcp-registry`.
- [ ] После мёржа — проверить, что `ai-lab` находится через `search_tools`.
...
- Деплой: отдельные PaaS-сервисы под `ai-lab-mcp` и под backend-движок.
- Нужно ли `ai-lab-mcp` проверять входящий `X-Auth-Identity` от Hub —
