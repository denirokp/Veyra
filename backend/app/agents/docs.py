...
MODE_INSTRUCTIONS: dict[ChatMode, str] = {
    ...
    ChatMode.full: (
        ...
        "ОСТАЛЬНЫЕ ПОЛЯ JSON:\n"
        "- facts: ОБЯЗАТЕЛЬНО 15-25 фактов (15 — минимум, иначе ответ "
        "считается некачественным). Каждый факт — КОНКРЕТНОЕ число с "
        "единицей+периодом, ИЛИ имя ответственного, ИЛИ конкретная дата/"
        "deadline, ИЛИ цитата из таблицы. Общие фразы типа «улучшение "
        "поддержки», «создание единого центра», «оптимизация процессов» — "
        "НЕ факты, их в facts ставить ЗАПРЕЩЕНО.\n"
        "- По каждому документу в источниках минимум 5-8 фактов.\n"
