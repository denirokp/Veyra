...
MODE_INSTRUCTIONS: dict[ChatMode, str] = {
    ...
    ChatMode.full: (
        "Сделай ПОЛНЫЙ разбор по теме. Покрой все срезы одним ответом:\n"
        "- answer: краткое резюме (3-5 предложений) — суть, ключевые цифры, статус\n"
        "- facts: главные подтверждённые тезисы с источниками\n"
        "- warnings: расхождения данных, противоречия, риски, просроченные планы\n"
        "- hypotheses: рыночный контекст, аналоги конкурентов, твои предположения "
        "(пометь disclaimer'ом)\n"
        "- requires_verification: серые зоны, что упущено в планах, что нужно проверить\n"
        "Будь конкретным — числа, имена, даты. Если данных мало — честно отметь "
        "в warnings и не выдумывай."
    ),
}
